...
The highest-leverage method for cmem-as-memory-tool, and now the top remaining lane. `thread/inject_items` (`common.rs`, handler `thread_inject_items` at `codex/codex-rs/app-server/src/request_processors/turn_processor.rs:175,802`) appends raw Responses API items to a loaded thread's model-visible history without starting a user turn (`{ threadId, items }` → `{}`). The flow: resume or fork an old thread through the bridge, inject a distilled "where you left off" block — decisions, open TODOs, key file paths, gotchas, built from cmem's catalog and annotations — then hand off to `codex resume`, so the next turn starts already primed with zero prompt-pasting. It also enables cross-thread memory: inject knowledge recovered from thread A into a fresh thread B. Touches: the `cmem continue` lane (`bin/cmem.js` `runContinueCommand`) plus `app-server-bridge.js` (`thread/resume` + `thread/inject_items`) and a new catalog builder that turns a session doc into that distilled handoff block.

**Why it is a dedicated stage, not tail-end work.** Two constraints make this heavier than the read lanes shipped so far. (1) It requires a *loaded* thread: `thread_inject_items` calls `load_thread`, which only succeeds if the thread was `thread/resume`-d earlier in the same app-server process. Our transport keeps one `codex app-server` child alive across requests, so `resume → inject` within one cmem invocation is tractable — but resume changes the bridge from stateless request/response to holding a live thread, which is new statefulness to get right (and to tear down cleanly). (2) It is a **write path that mutates a real thread's persisted history**, so — unlike every read lane — it cannot be safely live-verified against the user's real `~/.codex` data; it must be validated with a fake app-server plus an opt-in confirm before it ever touches a real thread. Ship it behind an explicit `--prime`/confirm flag, fake-server-tested, on a fresh forked thread by default so the source thread is never mutated. This is the next stage.
